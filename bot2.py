...
GROUP_CHAT_ID = "-1002865807540"  # Fixed: Changed from list to string
...
async def generate_excel(order: dict, apteka_info: dict, total_price: float, discounted_price: float, is_full_payment: bool):
    ...
    total_nds = 0
    total_with_nds = 0
    _, discountable_total = await get_total_price(order, bot.pool)
    ...
    for idx, (display_medicine, qty) in enumerate(order.items(), 1):
        ...
        if not price:
            price = 0
        final_price = price
        if is_full_payment and db_medicine not in non_discountable_medicines:
            if discountable_total <= 3_000_000:
                final_price = price * (1 - 0.05)
            elif discountable_total <= 6_000_000:
                portion_up_to_3m = min(price * qty, 3_000_000)
                portion_above_3m = max(0, (price * qty) - 3_000_000)
                final_price = (portion_up_to_3m * (1 - 0.05) + portion_above_3m * (1 - 0.08)) / qty
            else:
                portion_up_to_3m = min(price * qty, 3_000_000)
                portion_above_3m = min(max(0, (price * qty) - 3_000_000), 3_000_000)
                final_price = (portion_up_to_3m * (1 - 0.05) + portion_above_3m * (1 - 0.08)) / qty
        adjusted_price = (final_price * 100) / 112  # Price without VAT
...
async def handle_payment_type(message: Message, state: FSMContext):
    ...
    if is_full_payment and discountable_total > 0:
        if discountable_total <= 3_000_000:
            discount = discountable_total * 0.05
        elif discountable_total <= 6_000_000:
            discount = (3_000_000 * 0.05) + ((discountable_total - 3_000_000) * 0.08)
        else:
            discount = (3_000_000 * 0.05) + (3_000_000 * 0.08)
        discounted_price = full_total_price - discount
    logger.info(f"Discountable total: {discountable_total}, Discount: {discount}, Discounted price: {discounted_price}")
    ...
    await send_to_group(bot, order, apteka_info, total_price, discounted_price, is_full_payment, user_info)
    await message.answer(
